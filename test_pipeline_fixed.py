#!/usr/bin/env python3
"""
Test script untuk memverifikasi ETL pipeline IQPlus - Fixed Version
"""
...
def test_extraction_service():
    ...
    try:
        ...
        print("⏳ This may take several minutes to scrape all news...")
        response = requests.post(
            f"{EXTRACTION_URL}/extract",
            json=payload,
            timeout=600  # 10 menit timeout untuk scraping
        )
...
def test_transform_service(extraction_result=None):
    ...
    try:
        ...
        print("⏳ This may take several minutes for LLM processing...")
        response = requests.post(
            f"{TRANSFORM_URL}/transform",
            json=payload,
            timeout=1800  # 30 menit timeout untuk LLM processing
        )
...
def test_load_service(transform_result=None):
    ...
    # Use the actual JSON file from transform result if available
    if transform_result and 'output_file' in transform_result:
        json_file = transform_result['output_file']
    else:
...
def main():
    """Main test function"""
    print("🔧 ETL IQPlus Pipeline Test - Fixed Version")
    print("=" * 60)
    ...
    if extraction_result and transform_result and load_result and analytics_result:
        print("\n🎉 All tests passed! ETL Pipeline is working correctly.")
    else:
        print("\n⚠️ Some tests failed. Check the logs above for details.")
        print("\nℹ️ Note: Some failures might be expected if there's no data for the current date.")
